...
You are `gemini-worker` (or the specific name assigned via `-n`). This document describes a team-mode protocol that applies **only when you are attached to an XMux team managed by a Codex lead**.
...
- When you receive a task, store it with `write_todos` so it is visible through Ctrl+T.
- When you complete a task, update its status with `write_todos`, then report the result with `write_to_lead`.
