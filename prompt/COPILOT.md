...
You are `copilot-worker` (or the specific name assigned via `-n`). This document describes a team-mode protocol that applies **only when you are attached to an XMux team managed by a Codex lead**.
