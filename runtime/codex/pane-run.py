...
def parse_args():
    parser = argparse.ArgumentParser(description="XMux Codex TUI pane runner")
    parser.add_argument("--name", required=True)
    parser.add_argument("--project-dir", required=True)
    parser.add_argument("--state-dir", required=True)
    parser.add_argument("--socket", required=True)
    parser.add_argument("--codex-cmd", default=os.environ.get("XMUX_CODEX_TUI_CMD", "codex"))
...
def store_body(pending, request_id, response_nonce, body, digest):
    cleanup_pending(pending)
    request_id = str(request_id or "").strip()
    response_nonce = str(response_nonce or "").strip()
    ...
    if not response_nonce:
        return {"ok": False, "error": "missing response_nonce"}
    if not body:
...
def retrieve_body(pending, request_id, response_nonce):
    cleanup_pending(pending)
    request_id = str(request_id or "").strip()
    response_nonce = str(response_nonce or "").strip()
    ...
    if item.get("response_nonce") != response_nonce:
        return {"ok": False, "error": "response_nonce mismatch"}
...
def release_body(pending, request_id, response_nonce):
    request_id = str(request_id or "").strip()
    response_nonce = str(response_nonce or "").strip()
    item = pending.get(request_id)
    if not item:
...
def inject_prompt(master_fd, message):
    ...
    if message.get("clear", False):
        ...
        time.sleep(float(os.environ.get("XMUX_CODEX_CLEAR_DELAY", "0.05")))
    if not prompt:
        return
    ...
    if message.get("enter", True):
        time.sleep(float(os.environ.get("XMUX_CODEX_ENTER_DELAY", "0.12")))
...
def visible_response_prompt(message):
    ...
    if title:
        return f"[xmux-claude-response]\n\n{title}"
    return "[xmux-claude-response]"
...
def visible_request_prompt(message):
    ...
    title = str(message.get("title", "") or "").strip()
    if title:
        return f"[xmux-claude-request]\n\n{title}"
    return "[xmux-claude-request]"
...
def command_args(args):
    base = shlex.split(args.codex_cmd)
...
def main():
    ...
    if child_pid == 0:
        ...
        env = os.environ.copy()
        env["XMUX_CODEX_SESSION_NAME"] = args.name
        env["XMUX_CODEX_SOCKET"] = args.socket
                    ...
                    try:
                        ...
                        if message.get("type") == "ping":
                            ...
                            try:
                                ...
                                print(f"[xmux-codex pane-run] response injection failed: {inject_error}", file=sys.stderr)
                            ...
                            try:
                                ...
                                print(f"[xmux-codex pane-run] request injection failed: {inject_error}", file=sys.stderr)
                            ...
                            try:
                                ...
                                print(f"[xmux-codex pane-run] prompt injection failed: {inject_error}", file=sys.stderr)
                        else:
