...
`xmux gemini` refreshes `~/.gemini/settings.json` before starting Gemini so the
`xmux_bridge` MCP server points at this repo's `bridge-mcp-server.js`.

## Setup

Gemini CLI must be installed and authenticated. `xmux gemini` performs runtime
...
`xmux gemini` does not pin a concrete Gemini model. To select a model for XMux
Gemini panes, set `XMUX_GEMINI_MODEL`; XMux passes it to Gemini CLI as
`GEMINI_MODEL` unless the provider args already include `--model` or `-m`.

```bash
XMUX_GEMINI_MODEL=default xmux gemini -t <team>  # passes GEMINI_MODEL=auto
XMUX_GEMINI_MODEL=pro xmux gemini -t <team>
XMUX_GEMINI_MODEL=gemini-3.1-pro-preview xmux gemini -t <team>
